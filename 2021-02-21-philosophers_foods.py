...
def prep_one_word_foods(myfoods):
	one_words = []
	for mf in myfoods:
		mf = mf.lower().split(" ")
		for m in mf:
			m = slugify(m)
			if m not in one_words:
				one_words.append(m)
	return one_words
...
def main():
	...
	foodlist = prep_one_word_foods(foodlist)
